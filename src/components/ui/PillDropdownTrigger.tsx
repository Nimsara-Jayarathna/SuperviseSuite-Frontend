...
export function PillDropdownTrigger({
  icon,
  label,
  isOpen,
  disabled,
  ...
  return (
    <button
      ...
      className={cn(
        ...
        widthClassName,
        disabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer hover:ring-2 hover:ring-indigo-100',
        className,
      )}
      {...rest}
    >
      <span className="grid w-full grid-cols-[16px_1fr_16px] items-center gap-2 px-3 py-1.5">
        ...
        <span
          aria-hidden
          className={cn('flex h-4 w-4 items-center justify-center transition-transform duration-150', isOpen && 'rotate-180')}
        >
