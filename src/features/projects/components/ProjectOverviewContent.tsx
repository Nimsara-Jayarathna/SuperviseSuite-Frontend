...
export type ProjectOverviewTextField = 'title' | 'summary' | 'batch' | 'semester' | 'healthNote';
...
function buildFocusItems(role: OverviewRole, project: ProjectOverviewProject, summary: MilestoneSummary): string[] {
  if (summary.total === 0) {
        ...
        <div className="rounded-3xl border border-border bg-white p-6 shadow-sm transition-all hover:shadow-md">
          ...
          {edit?.isEditing && edit.form ? (
            <form className="mt-5 space-y-4" onSubmit={edit.onSubmit}>
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="space-y-1.5 sm:col-span-2">
                  ...
                  <textarea
                    maxLength={fieldLimits.healthNote}
                    rows={3}
                    value={edit.form.healthNote}
                    onChange={(event) => edit.onChangeField('healthNote', event.target.value)}
                    className="w-full rounded-2xl border border-border bg-white px-4 py-3 text-sm outline-none transition-colors focus:border-amber-300"
                  />
                </label>
              </div>
            ...
            <div className="mt-6 grid grid-cols-2 gap-6 sm:grid-cols-2">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Timeline risk</p>
                <div className="mt-1">
              ...
              <div className="col-span-2">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Health brief</p>
                <div
                  className={cn(
                    'mt-2 rounded-2xl border p-4',
                    healthBrief.autoGenerated ? riskToneClasses.panel : 'border-slate-200 bg-slate-50/35',
                  )}
                >
        ...
        <div className="rounded-3xl border border-border bg-white p-6 shadow-sm transition-all hover:shadow-md">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Total milestones</p>
            ...
            <div className={cn('rounded-2xl border p-4', completedToneClasses.card)}>
              <p className={cn('text-[10px] font-bold uppercase tracking-[0.2em]', completedToneClasses.label)}>
                Completed
              </p>
              <p className={cn('mt-2 text-2xl font-semibold', completedToneClasses.value)}>{summary.completed}</p>
            </div>
            <div className={cn('rounded-2xl border p-4', overdueToneClasses.card)}>
              <p className={cn('text-[10px] font-bold uppercase tracking-[0.2em]', overdueToneClasses.label)}>
                Overdue open
              </p>
              <p className={cn('mt-2 text-2xl font-semibold', overdueToneClasses.value)}>{summary.overdueOpen}</p>
            </div>
            <div className={cn('rounded-2xl border p-4', dueSoonToneClasses.card)}>
              <p className={cn('text-[10px] font-bold uppercase tracking-[0.2em]', dueSoonToneClasses.label)}>
                Due in 7 days
              </p>
              <p className={cn('mt-2 text-2xl font-semibold', dueSoonToneClasses.value)}>{summary.dueSoon}</p>
            </div>
          </div>
          ...
          {primaryMilestone ? (
            <div className="mt-5">
              <div className={cn('rounded-2xl border p-5', spotlightToneClasses.panel)}>
                <p className={cn('text-[10px] font-bold uppercase tracking-[0.2em]', spotlightToneClasses.label)}>
                ...
                <div className={cn('mt-3 flex items-center gap-2 text-sm font-medium', spotlightToneClasses.label)}>
                  <CalendarDays className="h-4 w-4" />
                  <span>Due {DATE_FORMATTER.format(parseLocalDate(primaryMilestone.milestone.dueDate))}</span>
                </div>
        ...
        <div className="rounded-3xl border border-border bg-white p-6 shadow-sm transition-all hover:shadow-md">
          ...
          <div className="mt-4 space-y-3 text-sm text-slate-600">
            <div className="flex items-start gap-2">
              <Clock3
                className={cn('mt-0.5 h-4 w-4', summary.inProgress > 0 ? 'text-sky-600' : 'text-slate-400')}
              />
              <p>
            ...
            <div className="flex items-start gap-2">
              <AlertTriangle
                className={cn('mt-0.5 h-4 w-4', summary.overdueOpen > 0 ? 'text-rose-600' : 'text-emerald-600')}
              />
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Files uploaded</p>
                  <p className="mt-1 text-base font-semibold text-slate-800">{filesUploadedCount}</p>
                  <p className="text-xs text-slate-500">{allowedFileTypesCount} allowed file types</p>
                </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Jira workspace</p>
                  <p className="mt-1 text-base font-semibold text-slate-800">
                    {jiraConnected ? 'Connected' : 'Not connected'}
                  </p>
                  <p className="text-xs text-slate-500">{jiraSyncStatusLabel}</p>
                </div>
                <PlugZap className={cn('h-4 w-4', jiraConnected ? 'text-emerald-600' : 'text-amber-600')} />
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Meetings</p>
                  <p className="mt-1 text-base font-semibold text-slate-800">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5 sm:col-span-2">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Meeting workflow</p>
                  <p className="mt-1 text-base font-semibold text-slate-800">
                    {meetingAnalytics.loading
                      ? 'Loading...'
